...
void main() {
  ...
  // This thread handles all Z values for position (pos.x, pos.y) in grid
  // gridIdx
  ...
  for (int localZ = 0; localZ < gridSize; localZ++) {
    ...
    if (stage == 0) {
      ...
      ivec3 dstPos;
      if (axis == 0)
        dstPos = ivec3(int(revIndex), pos.y, gridIdx * gridSize + localZ);
      else if (axis == 1)
        dstPos = ivec3(pos.x, int(revIndex), gridIdx * gridSize + localZ);
      else
        dstPos = ivec3(pos.x, pos.y, gridIdx * gridSize + int(revIndex));
